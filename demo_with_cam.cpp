...
int main(int argc, char **argv)
{
  ...
  */
  ...
  args.add_options()("camera-id,i", value<std::string>(&camera_id_str)->required(), "Camera index for live capture (0, 1...)");
  variables_map vm;
  try
  {
  ...
  {
    std::cerr << "Please input camera id" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  */
  ...
  camera.open(std::atoi(camera_id_str.c_str()));
  if (!camera.isOpened())
  {
    std::cerr << "Unable to open the camera : " << camera_id_str << std::endl;
    return EXIT_FAILURE;
  }
